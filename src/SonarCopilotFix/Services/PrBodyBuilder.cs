...
public sealed class PrBodyBuilder(IConfigurationHelper configurationHelper) : IPrBodyBuilder
{
    ...
    public string Build(IPullRequestResult summary)
    {
        StringBuilder builder = new();
        builder.AppendLine($"## Fix SonarQube issues for `{configurationHelper.SonarProjectKey}`");
        builder.AppendLine();
        builder.AppendLine("| Field | Value |");
        builder.AppendLine("| --- | --- |");
        builder.AppendLine($"| SonarQube project | `{configurationHelper.SonarProjectKey}` |");
