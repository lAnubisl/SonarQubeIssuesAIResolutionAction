    ...
    bool disposeClient = false) : ISonarQubeClient, IDisposable
{
    ...
    private string BuildIssueSearchUri(int page, int pageSize)
    {
        Dictionary<string, string?> query = new()
        {
            ["componentKeys"] = configurationHelper.InputComponents.Count > 0
                ...
                : configurationHelper.SonarProjectKey,
            ["p"] = page.ToString(),
    ...
    {
        string filePath = ExtractFilePath(dto.Component, configurationHelper.SonarProjectKey);
    ...
    {
        UriBuilder builder = new(new Uri(configurationHelper.SonarHostUri, "project/issues"))
        {
            Query = $"id={Uri.EscapeDataString(configurationHelper.SonarProjectKey)}&issues={Uri.EscapeDataString(issueKey ?? "")}&open={Uri.EscapeDataString(issueKey ?? "")}"
        };
        return builder.Uri;
    }
